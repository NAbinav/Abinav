...
import { faBars, faHighlighter, faHouse, faX } from '@fortawesome/free-solid-svg-icons';
...
const Page = () => {
  ...
  return (
    <section data-scroll-section style={{ height: '100dvh',width:"100%", justifyContent:"center", alignContent:"center",alignItems:"center"}}>
      <main id={menu?"show-menu":"hide-menu"}><Intro></Intro></main>
            <div id="menu">
                { (home)?
                  <FontAwesomeIcon icon={faX} className="sm:bg-grey-200 p-1 sm:p-2 md:bg-grey-400 " onClick={()=>{setmenu(!menu); sethome(!home)
                    window.scrollTo({
                      top:0
                    })
                  }}/>:
                  <FontAwesomeIcon icon={faBars} className="sm:bg-grey-200 p-1 sm:p-2 md:bg-grey-400 " onClick={()=>{setmenu(!menu); sethome(!home)
                  window.scrollTo({
                    top:0
                  })
                }}/>}
